...
<AppLayout>
  <div class="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow mt-10 print:bg-white print:text-black">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold print:text-black">Verified Bundles</h1>
    </div>
      ...
      <table class="min-w-full bg-white border border-gray-200 rounded-lg print:text-xs print:border-black">
        ...
        <tbody>
          <tr v-for="(bundle, idx) in bundles" :key="bundle.id" class="hover:bg-gray-50 print:hover:bg-white">
            ...
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.party_name ?? '-' }}</td>
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.lot ?? '-' }}</td>
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.cutsheet ?? '-' }}</td>
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.peice ?? '-' }}</td>
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.widht ?? '-' }}</td>
            <td class="px-4 py-2 border-b print:border-black">{{ bundle.jalilenght ?? '-' }}</td>
          ...
          <tr v-if="!bundles.length">
            <td colspan="12" class="text-center py-6 text-gray-500 print:text-black">No verified bundles found.</td>
          </tr>
        </tbody>
      </table>
    </div>
